...
import (
	...
	"github.com/vazha/gocryptotrader/currency/coinmarketcap"
...
func (bot *Engine) Start() error {
	...
	if bot.Settings.EnableDatabaseManager {
		if err := bot.DatabaseManager.Start(bot); err != nil {
			disabled := strings.Contains(err.Error(), "disabled")
			if disabled {
				gctlog.Warnf(gctlog.Global, "Database manager unable to start: %v", err)
			}else{
	...
	if bot.Settings.EnableCommsRelayer {
		if err := bot.CommsManager.Start(); err != nil {
			disabled := strings.Contains(err.Error(), "no communication relayers enabled")
			if disabled {
				gctlog.Warnf(gctlog.Global, "Communications manager unable to start: %v\n", err)
			}else{
